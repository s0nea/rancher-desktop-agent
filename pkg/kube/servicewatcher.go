...
func handleUpdate(oldObj, newObj interface{}, eventCh chan<- event, portTracker *tracker.PortTracker) {
	...
	if len(added) > 0 {
		...
		if existingPortMap := portTracker.Get(string(newSvc.UID)); existingPortMap != nil {
			err = updatePortMapping(added, existingPortMap, newSvc.UID, portTracker)
		} else {
			err = addPortMapping(added, newSvc.UID, portTracker)
		}
...
func addPortMapping(ports map[int32]corev1.Protocol, serviceUID types.UID, portTracker *tracker.PortTracker) error {
	portMapAdd, err := createPortMapping(ports)
	if err != nil {
	...
	log.Debugf("adding %d port mappings for service ID %s", len(ports), string(serviceUID))

	if err = portTracker.Add(string(serviceUID), portMapAdd); err != nil {
...
func removePortMapping(serviceUID types.UID, portTracker *tracker.PortTracker) error {
	...
	err := portTracker.Remove(string(serviceUID))
	if err != nil {
...
func updatePortMapping(
	...
	newPortMap, err := createPortMapping(ports)
	if err != nil {
		log.Errorf("creating initial port mapping failed: %v", err)
	...
	if !reflect.DeepEqual(newPortMap, existingPortMap) {
		err = removePortMapping(serviceUID, portTracker)
		if err == nil {
			err = addPortMapping(ports, serviceUID, portTracker)
		}
	}
